...
class ShopwareProcess
{
    ...
    public function getAllProductsAsXmlArray()
    {
        ...
        // Get root category of selected shop
        $shopRootCategory = $this->shop->getCategory()->getId();

        // Get all articles from selected shop
        $allArticles = $this->shop->getCategory()->getAllArticles();

        // Get all Categories from selected shop
        ...
        foreach ($allArticles as $article) {
            $findologicArticle = new FindologicArticleModel($article, $this->shopKey, $allUserGroups);
